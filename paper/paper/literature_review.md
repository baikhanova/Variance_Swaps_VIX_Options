...
Initially released as a Goldman Sachs Quantitative Strategies Research Note, Demeterfi et al. (1999) is generally recognized as the one of the first major papers on variance swaps. Before this paper, volatility was only traded indirectly, via option positions where volatility exposure was combined with delta and gamma risk. Demeterfi et al. proved that one can obtain a pure exposure to realised variance through a model-free replication method.

### 2.2 The Log-Contract Replication Method

Demeterfi et al.'s main idea is that the payoff of a variance swap, which is the difference between the realised variance and the strike, can be replicated using a portfolio of European puts and calls over the entire strike range that can be rebalanced continuously. More precisely, one needs $2/K^2$ units of each option at strike $K$, which combined together form a log-contract: a contract that pays out
...
Understanding the log-contract argument is therefore a necessary step before deriving other important results in the project, such as the VIX formula and the fair-strike calculation under Heston (Equation 2), and the model-free hedging strategy in W8.
...
The main empirical finding is that the VRP is on average positive, typically around two to four annualised volatility points, and highly stable over time and across different equity indices. This premium represents the compensation investors demand for taking on the risk of unexpected increases in variance, which represents a non-diversifiable risk and that spikes sharply during market dislocations.
...
Carr and Wu's finding that a significant alpha remains after controlling for equity and volatility betas provides the theoretical basis for treating the VRP as a real risk premium rather than simply another form of equity beta. This is one of the key questions addressed in W8.
...
The Heston model is a **one-factor stochastic volatility model**: a single hidden process $v_t$ governs both the instantaneous variance of SPX returns and the level of the VIX. Under Heston, the squared VIX is an affine function of $v_t$ (Equation 4 of the project), which means that the VIX futures term structure is entirely determined by $\kappa$, $\bar{v}$, and $v_0$, which are also the parameters describing the SPX implied volatility surface.
...
This is Equation (4) of the project specification. The affinity property is critical because it establishes that $\text{VIX}_t$ is a direct transformation of $v_t$ under Heston. This feature simultaneously enables tractable VIX option pricing *and* exposes the one-factor constraint that prevents joint calibration.
...
The four sources reviewed above do not merely coexist in the literature, they form a connected sequence of ideas. Demeterfi et al. (1999) establish that variance is a replicable, tradeable quantity and define its fair price in a model-free manner. Carr and Wu (2009) demonstrate empirically that the market price of variance systematically exceeds its realised value, establishing the variance risk premium as a robust and economically significant phenomenon. Bergomi (2005) identifies the structural reason why the dominant one-factor model, Heston, cannot simultaneously account for the SPX surface and the VIX term structure, thereby motivating the forward variance and multi-factor extensions that define the current direction of research. Gatheral (2006) provides the mathematical tools, including the affinity result, the CIR characteristic function, and the smile analysis, which enable tractable analytical work within the Heston framework while also exposing its limitations.

This progression, from replication and empirical measurement to model critique and analytical tools, mirrors the structure of the present project and provides the classical foundations upon which the contemporary literature reviewed in the second part builds.
